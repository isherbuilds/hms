...
export const Route = createFileRoute("/$orgSlug/billing/invoices/$invoiceId/receipt/$paymentId")({
  loader: async ({ context: { queryClient }, params: { orgSlug, invoiceId, paymentId } }) => {
    ...
    return {
      documentNumber: data.payments.find((payment) => payment.id === paymentId)?.receiptNumber,
    };
  },
  head: ({ loaderData }) => ({
    meta: [{ title: `${loaderData?.documentNumber ?? "Receipt"} · Receipt · HMS` }],
  }),
    ...
    <>
      ...
      >
        ...
        <dl className="grid grid-cols-[36mm_1fr] gap-x-4 gap-y-2 border-y border-black py-4">
          ...
          <dd>{formatDateTime(payment.createdAt, timeZone)}</dd>
