...
Multi-tenant B2B SaaS base. Bun + Turborepo; TanStack Start (`apps/web`) + Hono/oRPC (`apps/server`); Drizzle + PostgreSQL; Better Auth with the organization plugin.
...
- Architecture: [overview](./docs/contributing/architecture/index.md) — [tenancy](./docs/contributing/architecture/tenancy.md), [authorization](./docs/contributing/architecture/authorization.md), [request lifecycle](./docs/contributing/architecture/request-lifecycle.md), [data storage](./docs/contributing/architecture/data-storage.md), [audit](./docs/contributing/architecture/audit.md), [file storage](./docs/contributing/architecture/file-storage.md)
...
2. **Org context is explicit procedure input, proven by the permission guard.** Org pages pass their `/org/:orgSlug` route param as `input.orgSlug` through the single `/rpc` client. Framework adapters supply only request dependencies (`session` and `headers`); `orgProcedure(permission, input)` resolves membership directly in its internal guard and turns the claim into verified `context.scope`. The permission is a required constructor argument and the raw builder is not exported, so an org procedure cannot omit the guard. Handlers use only scope for authorization and SQL. Membership is uncached across requests, and there is no fallback to `session.activeOrganizationId`. Sign-up is disabled: accounts are created by an operator via `createUserWithPassword` / `scripts/create-user.ts`, never through a public endpoint.
