...
2. **Org context is explicit procedure input, proven by the permission guard.** Org pages pass their `/org/:orgSlug` route param as `input.orgSlug` through the single `/rpc` client. Framework adapters supply only request dependencies (`session` and `headers`); `requirePermission` resolves membership directly and turns the claim into verified `context.scope`. Handlers use only scope for authorization and SQL. Membership is uncached across requests, and there is no fallback to `session.activeOrganizationId`. Sign-up is disabled: accounts are created by an operator via `createUserWithPassword` / `scripts/create-user.ts`, never through a public endpoint.
